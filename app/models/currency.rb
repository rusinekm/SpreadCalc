class Currency < ActiveRecord::Base
  ...
  def set_best_value(statnodes)
    if statnodes != [] then
      self.update(last_best_buy_statnode_id: current_buy_value_node(statnodes).id, last_best_sell_statnode_id: current_sell_value_node(statnodes).id, average_value: (statnodes.sum {|node| node.net_worth/statnodes.count}))
    else
      self.update(last_best_buy_statnode_id: nil, last_best_sell_statnode_id: nil, average_value: 0)
    end
  end
